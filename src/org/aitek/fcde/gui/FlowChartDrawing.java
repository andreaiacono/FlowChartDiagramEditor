...
public class FlowChartDrawing {
	...
	private void createGraphicalBlocks() {
		...
		for (String id : flowChart.getBlockIds()) {
			...
			for (Connection connection : block.getConnections()) {
				...
				// logger.severe("cerated conn from " + graphicalBlock.getId() + " to " +
				// destinationGraphicalBlock);
			}
		}
	}
	...
	/**
	 * recursive method for putting a block (and its following blocks) inside the grid.
	 * 
	 * @param currentBlock
	 * @param currentRow
	 * @param currentColumn
	...
	public void draw(Graphics2D g2d) {
		...
		for (String key : graphicalBlocks.keySet()) {
			...
			GraphicalBlock block = getGraphicalBlock(key);
			for (GraphicalConnection graphicalConnection : block.getGraphicalConnections()) {
				graphicalConnection.setIsPainted(false);
			}
		}
	}
