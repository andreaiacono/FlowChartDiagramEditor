...
public class StringUtils {
    ...
    /**
     * transforms every occurrence of a quote character "'" into a double quote
     * "''" for SQL queries
     *
    ...
    /**
     * transforms a string into a string of the representation int hexadecimal
     * format
     *
    ...
    /**
     * transforms an array of byte into a string of the representation int
     * hexadecimal format
     *
     * @param bt the bytes to be converted
     * @param n the number of bytes per row to be displayed
     ...
     * @return the object created public static DatabaseAdmin initServlet() {
     * //throws DbAdminException { return new DatabaseAdmin(false); } /**
     * initalizes the servlet creating the DatabaseAdmin objet
     * @param strConfigFile the path and filename of the configuration file
     * @return the object created public static DatabaseAdmin initServlet(String
     * strConfigFile) throws DbAdminException { // returns the created
     * DatabaseAdmin object return new DatabaseAdmin(strConfigFile); }
     */
    public static String getWord(String strTotal) {
        ...
        if (pos != -1) {
            return strTotal.substring(0, pos);
        }
    ...
    /**
     * gets the value of the specified fields from the resultset and
     * concatenates them to the output string
     *
     * @param rs
     * @param strFields
     * @param strSeparator
    ...
    /**
     * transform the specified string into a string that can fit into java
     * string declaration (removes linefeeds, backscapes quotes, ..
     *
     * @param strVal
     * @return
     */
     ...
     *
     * @param strQuery
     * @param strHeader an optional parameter fot setting a text before the
     * query (if null is not considered)
    ...
    public static ArrayList<String> splitString(String strQuery, String strHeader, Graphics g, Font fn, int maxwidth) {
        ...
        while (cnt < strQuery.length()) {
            ...
            if (cnt < strQuery.length()) {
                while (strQuery.charAt(cnt) != ' ' && cnt >= 0) {
                    cnt--;
                }
            }
     ...
     * @return a string containing new lines characters every at most "char"
     * character
     */
    public static String insertStringEvery(String strText, String strToInsert, int chars) {
        ...
        while (cnt < sbText.length()) {
            ...
            if (cnt >= sbText.length() || sbText.length() > 1000) {
                break;
            }
    ...
    /**
     * gets an AL of string values and transofrms them into a string with the
     * declaration of a string array (e.g. String[] strTest = { "val1", "val2"}
     *
     * @param alNames
    ...
    public static String insertValueInString(String strSource, String strValue, String strSeparator, int maxValues) {
        ...
        if (al.size() == maxValues) {
            strSource = strSource.substring(strSource.indexOf(strSeparator));
        }

        // if is not present, insert the value passed
        if (!isPresent) {
            strSource = strSource + strValue + strSeparator;
        }
     ...
     * @param alValues the values to be concatenated
     * @param strSeparator the separator between one element and the other
     * @param strFixedValue a value to be appended to each value of the
     * ARRAYLIST
     * @return the string with the values seaparated by the strSeparator
     */
    ...
    /**
     * separate all the uppercase characters of the specified string from the
     * preceding character using the spcecifed separator
     *
    ...
    public static String separateUpperCaseLetters(String strValue, String strSeparator) {
        ...
        for (int j = 0; j < strValue.length(); j++) {
            ...
            if (j > 0) {
                if (strValue.toLowerCase().charAt(j - 1) == strValue.charAt(j - 1) && strValue.toUpperCase().charAt(j) == strValue.charAt(j)) {
                    sbValue.append(strSeparator);
                }
            }
    ...
    public static String getValuesInHtml(Properties prValues) {
        ...
        if (prValues.size() == 0) {
            return "<html>No Values present</html>";
        }
    ...
    /**
     * returns a string with all the parenthesis content of the specified string
     * removed (parenthesis included)
     *
     * @param strValue
     * @return
     */
    public static String removeParenthesisContent(String strValue) {
        ...
        for (int j = 0; j < strValue.length(); j++) {
            ...
            if (p == 0 && strValue.charAt(j) != ')') {
                sbRemoved.append(strValue.charAt(j));
            }
        }
    ...
    public static String removeLineComments(String strScript, String strComment) throws Exception {
        ...
        while ((strLine = br.readLine()) != null) {
            ...
            // before the comment
            if (strLine.indexOf(strComment) >= 0) {
                sbNew.append(strLine.substring(0, strLine.indexOf(strComment)) + "\n");
            }
            // else, appends the whole line
            else {
                sbNew.append(strLine + "\n");
            }
        }
